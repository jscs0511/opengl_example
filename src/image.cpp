...
bool Image::LoadWithStb(const std::string& filepath) {
    ...
    //m_data[0]~m_data[2] : first fixel RGB, m_data[3]~m_data[5] : second fixel RGB, ...
    //if not NULL, we succeed to get image data into CPU
    if (!m_data) {
        SPDLOG_ERROR("failed to  bload image: {}", filepath);
        return false;
    }
    return true;
}
     ...
     for (int j = 0; j < m_height; j++){
        for (int i = 0; i < m_width; i++){
            ...
            for (int k=0; k < m_channelCount; k++)
                m_data[pos + k] = value;
            if (m_channelCount>3)//if alpha channel exists
                m_data[3] = 255;//if alpha channel is 0, the result image is transparent
        }
     }
}